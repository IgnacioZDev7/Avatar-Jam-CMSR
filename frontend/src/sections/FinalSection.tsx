  ...
  return (
    ...
    >
      <motion.div style={{ y: imageY }} className="absolute -inset-x-8 -inset-y-20 z-0">
        <img src={whiteTerritory} alt="" className="h-full w-full object-cover object-center opacity-72" />
      ...
      >
        ...
        <header className="flex flex-col items-center gap-12">
          ...
          <motion.p variants={FADE_IN_UP} className="max-w-3xl text-xl font-light italic leading-relaxed text-white/72 md:text-2xl">
        ...
        <motion.div variants={FADE_IN_UP} className="grid w-full grid-cols-1 gap-4 text-left md:grid-cols-4">
          {['Memoria', 'Identidad', 'Cultura', 'Dignidad'].map((word) => (
            <div key={word} className="border border-white/12 bg-black/20 p-5 backdrop-blur-sm">
              <span className="text-[10px] font-black uppercase tracking-[0.5em] text-white/62">{word}</span>
            </div>
          ))}
        </motion.div>
        ...
        <motion.div variants={FADE_IN_UP} className="flex flex-col items-center gap-12">
          <div className="flex flex-col items-center gap-3 opacity-55">
            <span className="text-[10px] font-black uppercase tracking-[1em] text-white/70">Fin de la Transmision</span>
          ...
          <button
            ...
            className="group relative border border-white/20 bg-black/20 px-10 py-7 transition-all duration-700 ease-out hover:border-white/80 active:scale-95 md:px-16"
          >
      ...
      <div className="pointer-events-none absolute inset-0 z-20 shadow-[inset_0_0_50vw_rgba(0,0,0,0.92)]" />
    </section>
  );
};
